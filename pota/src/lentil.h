...
public:
    ...
    void setup_camera (AtUniverse *universe) {
        ...
        if (bokeh_enable_image && !image.read(bokeh_image_path.c_str())){
            AiMsgError("[LENTIL CAMERA] Could not open bokeh image.");
            ...
            if (crypto_node){
                ...
                while (!crypto_data->is_setup_completed) {
                    ...
                    if (time_cnt == 500) { // guess that crypto is in the same queue, behind lentil
                        ...
                        AiMsgInfo("[LENTIL] Timed out waiting for cryptomatte setup.");
                        break;
                    }
                }
    ...
    void setup_lentil_aovs(AtUniverse *universe) {
        ...
        if (!lentil_operator_node) {
            AiMsgError("[LENTIL] Missing operator: lentil_operator.");
        }
...
private:
    ...
    void destroy_buffers() {
     ...
     bool get_bidirectional_status(AtUniverse *universe) {
        ...
        if (!enable_dof) {
            AiMsgWarning("[LENTIL BIDIRECTIONAL] Depth of field is disabled, disabling bidirectional sampling.");
            return false;
        }
    ...
    void camera_model_specific_setup () {
        ...
        switch (cameraType){
            case PolynomialOptics:
            {
                ...
                AiMsgInfo("[LENTIL CAMERA] ----------  LENS CONSTANTS  -----------");
                AiMsgInfo("[LENTIL CAMERA] Lens name: %s", lens_name);
                AiMsgInfo("[LENTIL CAMERA] Lens f-stop: %f", lens_fstop);
                #ifdef DEBUG_LOG
                AiMsgInfo("[LENTIL CAMERA] lens_outer_pupil_radius: %f", lens_outer_pupil_radius);
                AiMsgInfo("[LENTIL CAMERA] lens_inner_pupil_radius: %f", lens_inner_pupil_radius);
                AiMsgInfo("[LENTIL CAMERA] lens_length: %f", lens_length);
                AiMsgInfo("[LENTIL CAMERA] lens_back_focal_length: %f", lens_back_focal_length);
                AiMsgInfo("[LENTIL CAMERA] lens_effective_focal_length: %f", lens_effective_focal_length);
                AiMsgInfo("[LENTIL CAMERA] lens_aperture_pos: %f", lens_aperture_pos);
                AiMsgInfo("[LENTIL CAMERA] lens_aperture_housing_radius: %f", lens_aperture_housing_radius);
                AiMsgInfo("[LENTIL CAMERA] lens_inner_pupil_curvature_radius: %f", lens_inner_pupil_curvature_radius);
                AiMsgInfo("[LENTIL CAMERA] lens_outer_pupil_curvature_radius: %f", lens_outer_pupil_curvature_radius);
                AiMsgInfo("[LENTIL CAMERA] lens_inner_pupil_geometry: %s", lens_inner_pupil_geometry.c_str());
                AiMsgInfo("[LENTIL CAMERA] lens_outer_pupil_geometry: %s", lens_outer_pupil_geometry.c_str());
                AiMsgInfo("[LENTIL CAMERA] lens_field_of_view: %f", lens_field_of_view);
                AiMsgInfo("[LENTIL CAMERA] lens_aperture_radius_at_fstop: %f", lens_aperture_radius_at_fstop);
                #endif
                AiMsgInfo("[LENTIL CAMERA] --------------------------------------");


                
                AiMsgInfo("[LENTIL CAMERA] Wavelength: %f nm", lambda);
                ...
                if (input_fstop == 0.0) {
                    ...
                    AiMsgInfo("[LENTIL CAMERA] Calculated f-stop: %f", calculated_fstop);
                    AiMsgInfo("[LENTIL CAMERA] Calculated aperture radius: %f mm", calculated_aperture_radius);
                    
                    aperture_radius = std::min(lens_aperture_radius_at_fstop, calculated_aperture_radius);
                }

                AiMsgInfo("[LENTIL CAMERA] Lens wide open f-stop: %f", lens_fstop);
                AiMsgInfo("[LENTIL CAMERA] Lens wide open aperture radius: %f mm", lens_aperture_radius_at_fstop);
                AiMsgInfo("[LENTIL CAMERA] F-stop-calculated aperture radius: %f mm", aperture_radius);
                AiMsgInfo("[LENTIL CAMERA] --------------------------------------");


                AiMsgInfo("[LENTIL CAMERA] User-supplied focus distance: %f mm", focus_distance);

                /*
                AiMsgInfo("[LENTIL CAMERA] calculating sensor shift at focus distance:");
                sensor_shift = camera_set_focus(focus_distance, po);
                AiMsgInfo("[LENTIL CAMERA] sensor_shift to focus at %f: %f", focus_distance, sensor_shift);
                */

                // logartihmic focus search
                double best_sensor_shift = logarithmic_focus_search(focus_distance);
                AiMsgInfo("[LENTIL CAMERA] Sensor shift using logarithmic search: %f mm", best_sensor_shift);
                ...
                AiMsgInfo("[LENTIL CAMERA] sensor_shift [average guesses backwards light tracing] to focus at infinity: %f", infinity_focus_sensor_shift);
                */

                // logarithmic infinity focus search
                double best_sensor_shift_infinity = logarithmic_focus_search(999999999.0);
                AiMsgInfo("[LENTIL CAMERA] Sensor shift (logarithmic forward tracing) to focus at infinity: %f mm", best_sensor_shift_infinity);
                    
                // bidirectional parallel infinity focus search
                double infinity_focus_parallel_light_tracing = camera_set_focus_infinity();
                AiMsgInfo("[LENTIL CAMERA] Sensor shift (parallel backwards light tracing) to focus at infinity: %f mm", infinity_focus_parallel_light_tracing);
                ...
                AiMsgInfo("[LENTIL CAMERA] Focus test ray: %f mm", test_focus_distance);
                if(!focus_test){
                    AiMsgWarning("[LENTIL CAMERA] Focus check failed. Either the lens system is not correct or the sensor is placed at the wrong distance.");
                }
